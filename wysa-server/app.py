...
def login():
    ...
    if user is not None:
        is_valid_password = verify_password(password,user["salt"], user["hashed_password"])
        ...
        user_status = user_collection.insert_one({"username" : username, "salt" : salt, "hashed_password" : hashed_password})
    ...
    if auth_header:
        ...
        try:
            ...
            if sleep_struggle_period == "<2" :
                ...
                score = 2
            sleep_assessment_collection.insert_one({"username" : username, "sleep_struggle_period" : sleep_struggle_period, "score" : score})
            return jsonify({"status" : "success"}),200
        except Exception as e:
            print(e)
            return jsonify({"status" : "error","message" : "An unexpected error occurred on the server side"}),500
...
def handle_sleep_time():
    ...
    if auth_header:
        ...
        try:
            ...
            sleep_assessment_collection.update_one({"username" : username}, {"$set" : {"sleeping_time" : sleeping_time}})
            ...
            return jsonify({"status" : "success", "estimated_waking_time" : waking_time}),200
        except Exception as e:
            return jsonify({"status" : "error","message" : "An unexpected error occurred on the server side"}),500
...
def handle_waking_time():
    ...
    if auth_header:
        ...
        try:
            ...
            sleep_assessment_collection.update_one({"username" : username}, {"$set" : {"waking_time" : waking_time}})
            ...
            return jsonify({"status" : "success", "estimated_sleep_hours" : sleep_hours}),200
        except Exception as e:
            print(e)
            return jsonify({"status" : "error","message" : "An unexpected error occurred on the server side"}),500
    ...
    if auth_header:
        ...
        try:
            ...
            if sleep_hours < 6:
                ...
                score = 3
            db_score = sleep_assessment_collection.find_one({"username" : username})['score']
            avg_score =int((db_score + score) / 2)
            sleep_assessment_collection.update_one({"username" : username}, {"$set" : {"sleep_hours" : sleep_hours, "score" : avg_score}})
